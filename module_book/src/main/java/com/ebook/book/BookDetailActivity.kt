...
class BookDetailActivity : BaseMvvmActivity<ActivityDetailBinding, BookDetailViewModel>() {
    ...
    override fun onBindViewModel(): Class<BookDetailViewModel> {
    ...
    fun hadRemoveBook(value: BookShelf) {
        for (i in mViewModel.bookShelfList.indices) {
            if (mViewModel.bookShelfList[i].noteUrl == value.noteUrl) {
                mViewModel.bookShelfList.removeAt(i)
                break
            }
        }
        if ((null != mViewModel.mBookShelf && value.noteUrl == mViewModel.mBookShelf!!.noteUrl) ||
            ...
            mViewModel.inBookShelf = false
            if (null != mViewModel.searchBook) {
                mViewModel.searchBook!!.add = false
            }
            updateView()
        }
    }
